...
async function generatePrompt(skeleton, actCat, sceneCat, themeCat, roleCat) {
  ...
  const userMsg = `Generate a detailed photorealistic NSFW image generation prompt using this scene skeleton:\n${skeletonStr}${emphasisLine}${castOverride}${cumRule}${soloNoCum}${cameraAngleRule}${hairColorRule}${expressionRule}\n\nIMPORTANT: Use the EXACT setting, subject, race, body_type, role, theme, hair_color, facial_expression, and camera_angle. Expand into a rich, explicit prompt.`;
